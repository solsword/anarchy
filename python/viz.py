...
def demo_shuffle():
  """
  Creates images that demonstrate the shuffle functionality and compare it
  against irreversible shuffling using the built-in random module.
  """
  seed = 8231093281
  random.seed(seed)
  ...
  for i in range(20):
    ...
    seq_a_new = [
      seq_a[anarchy.cohort_shuffle(j, 100, seed)]
  ...
  for i in range(20):
    ...
    seq_a_here = [
      seq_a[anarchy.cohort_shuffle(j, 100, seed + i)]
        for j in range(len(seq_a))
    ]
  ...
  for i in range(N_TRIALS):
    shuf_a = [
      seq_a[anarchy.cohort_shuffle(j, 50, seed + i)]
        for j in range(len(seq_a))
    ]
...
def demo_distribute():
  """
  Creates images that demonstrate the distribute functionality and compare it
  against irreversible distribution using the built-in random module.
  """
  seed = 3129320881
  random.seed(seed)
  ...
  for y in range(20):
    ...
    for item in range(50):
      s = anarchy.distribution_segment(item, 50, 10, 10, 0.5, seed + y)
  ...
  for i, r in enumerate([0, 0.1, 0.3, 0.5, 0.7, 0.9, 1]):
    ...
    for y in range(start, start + 5):
      ...
      for i in range(10):
        segments.append([])
      for item in range(50):
        s = anarchy.distribution_segment(item, 50, 10, 10, r, seed + y)
        segments[s].append(item)
  ...
  for y in range(76, 86):
    ys = seed + y
    for x in range(100):
      here = anarchy.distribution_portion(x, 5000, 100, 100, 0.2, ys)
  ...
  for y in range(90, 100):
    ys = seed + y
    for x in range(100):
      here = anarchy.distribution_portion(x, 5000, 100, 100, 0.8, ys)
      val = round(here / 100 * 255)
      a_demo.putpixel((x, y), (val,)*3)
